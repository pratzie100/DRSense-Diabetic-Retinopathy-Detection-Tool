...
CORS(app, resources={
    r"/*": {
        "origins": ["https://drsense-pratyush-kargeti.netlify.app"],
        "methods": ["GET", "POST", "OPTIONS"],
        "allow_headers": ["Content-Type", "Authorization"]
    }
})
...
@app.route('/')
def home():
    response = jsonify({'message': 'DRSense API is up and running!!'})
    response.headers['Access-Control-Allow-Origin'] = 'https://drsense-pratyush-kargeti.netlify.app'
    return response
...
def preprocess():
    ...
    try:
        ...
        if file.filename == '':
            ...
            return response
        _, base64_image = preprocess_image(file, return_image=True)
        response = jsonify({'preprocessed_image': f'data:image/png;base64,{base64_image}'})
        logging.info(f"/preprocess completed in {time.time() - start_time} seconds")
        return response
    except Exception as e:
        logging.error(f"Error in preprocess: {str(e)}")
        response = jsonify({'error': f'Preprocessing failed: {str(e)}'})
...
def predict():
    ...
    try:
        ...
        logging.info(f"/predict completed in {time.time() - start_time} seconds")
        return response
    except Exception as e:
        logging.error(f"Error in predict: {str(e)}")
        response = jsonify({'error': f'Prediction failed: {str(e)}'})
        response.status_code = 500
        return response
    finally:
        if response:
...
def generate_report():
    start_time = time.time()
    logging.info("Starting /generate_report")
    response = None
    try:
        data = request.json
        required_keys = ['original_image', 'preprocessed_image', 'prediction', 'advice', 'filename']
        for key in required_keys:
            if key not in data:
                response = jsonify({'error': f'Missing {key}'})
                response.status_code = 400
                return response
        ...
        logging.info(f"/generate_report completed in {time.time() - start_time} seconds")
        return response
    except Exception as e:
        logging.error(f"Error generating report: {str(e)}")
        response = jsonify({'error': f'Report generation failed: {str(e)}'})
        response.status_code = 500
        return response
    finally:
        if response:
